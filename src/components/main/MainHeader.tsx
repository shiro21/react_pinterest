...
export const MainHeader = ({ auth, setPopularData, setSearchValue }: { auth: UserProps, setPopularData: Function, setSearchValue: Function }) => {
    ...
    const [searchHistory, setSearchHistory] = useState<PinsProps[]>([]);
    ...
    useEffect(() => {
        if (search === "") {
            ...
            api.post("/search/realTimeSearch", {search: search})
            .then(res => {
                if (res.data.code === "y") setSearchHistory(res.data.data);
            })
            ...
            <div className={styles.items_wrap}>
                ...
                <div className={`${styles.items} ${styles.items_full}`}>
                    <div className={styles.input_wrap}>
                        ...
                        <input type="text" name="search" id="search" value={search} onFocus={searchFocus} placeholder="검색" onChange={onSearch}onKeyPress={onSeachEnter} />
                    </div>
                    {
